...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from semilearn.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet
    from semilearn.datasets import get_cifarstl, get_pacs, get_domainnet, get_domainnet_balanced
    ...
    if dataset == "eurosat":
        ...
        test_dset = None
    elif dataset == 'cifarstl':
        lb_dset, ulb_dset, eval_dset, test_dset = get_cifarstl(args, algorithm, num_labels, data_dir=data_dir)
    elif dataset == 'pacs':
        lb_dset, ulb_dset, eval_dset, out_dset, test_dset = get_pacs(args, algorithm, num_labels, data_dir=data_dir)
    elif dataset == 'domainnet':
        lb_dset, ulb_dset, eval_dset, out_dset, test_dset = get_domainnet(args, algorithm, num_labels, data_dir=data_dir)
    elif dataset == 'domainnet_balanced':
        lb_dset, ulb_dset, eval_dset, out_dset, test_dset = get_domainnet_balanced(args, algorithm, num_labels, data_dir=data_dir)
    else:
        return None
    
    dataset_dict = {'train_lb': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset, 'out': out_dset, 'test': test_dset}
    return dataset_dict
...
def get_data_loader(args,
    ...
    if isinstance(data_sampler, str):
        ...
        return DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=collate_fn,
        ...
        return DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
                          collate_fn=collate_fn, pin_memory=pin_memory, sampler=data_sampler,
                          generator=generator, drop_last=drop_last)
